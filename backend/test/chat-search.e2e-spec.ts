...
describe('Message search (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const member = await prisma.user.create({
      data: { email: `search-a-${stamp}@example.com`, name: 'SearchA', password: 'x' },
    });
    const outsider = await prisma.user.create({
      data: { email: `search-o-${stamp}@example.com`, name: 'SearchO', password: 'x' },
    });
    memberId = member.id;
    outsiderId = outsider.id;

    const group = await prisma.group.create({ data: { name: 'Search Group', createdBy: memberId } });
    groupId = group.id;
    await prisma.groupMember.create({ data: { groupId, userId: memberId, role: 'OWNER' } });

    await prisma.message.createMany({
      data: [
        { groupId, senderId: memberId, content: 'Let us deploy the API today', type: 'USER' },
        { groupId, senderId: memberId, content: 'The DEPLOY pipeline is green', type: 'USER' },
        { groupId, senderId: memberId, content: 'lunch plans?', type: 'USER' },
        { groupId, senderId: memberId, content: 'deploy this later', type: 'USER', deletedAt: new Date() },
        { groupId, senderId: null, content: 'Daily summary: deploy discussed', type: 'AI_SUMMARY' },
      ],
    });
